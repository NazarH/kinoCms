...
<div class='wrapper' id='wrapp'>
   ...
   <div class='content'>
      <button style='display: none;' @click='imgSwipe()' id='btnSwipe'></button>
      <div class='imgBanner'>
         <img src='' id='imgSwipe'>
      </div>
      <div>
         <div class='content__left'>
            ...
            <div class='some-blocks'>
              <div class='stockInfo'>
               <img :src='stocksData.newsMainImg'>
              </div>
              <div class='newsDescr'>
              {{stocksData.newsDescr}}
               </div>

               
            </div>
            <div class='ytVideo'>
                 <iframe width="1000" height="515" :src="stocksData.newsLink" 
   ...
   components:{
    ...
    created(){
       if(this.$route.params.data !== undefined){
           this.stocksData = JSON.parse(this.$route.params.data);

           const db = getDatabase();
           set(fireRef(db, `stocks-temp/0`), this.stocksData);
...
loginMeth(){
      ...
      searchFilm(){
       ...
       getData2(film){
        ...
        imgSwipe(){
         setTimeout(() => {
            document.getElementById('imgSwipe').src=`${this.stocksData.newsGalary[this.counter].img}`;
            document.getElementById('btnSwipe').click();
            setTimeout(() => {
               this.counter = this.counter + 1;
            }, 2000);
            setTimeout(() => {
               if(this.counter === this.stocksData.newsGalary.length){
               this.counter = 0;
               }
            }, 2500);
         }, 3000);
       ...
       getUserFirstFunc(){
           ...
           get(child(dbRef, `users/`)).then((snapshot) => {
            ...
            get(child(dbRef, `stocks-temp/`)).then((snapshot) => {
            var banners = [];
            snapshot.forEach(element => {
              banners.push(element.val());
            });
            this.stocksData = banners[0];
...
/* ---------------- 0 */
   ...
   .stock{
    ...
    .content__right>img:first-child{
        margin-top: 25px;
    }

    .content__right>img:last-child{
        margin-bottom: 25px;
    }
