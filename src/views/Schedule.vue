...
<div class='wrapper' id='wrapp'>
   ...
   <div class='sort'>
      <select style='display: none;'>
         <option>Дата</option>
         <option v-for='date in filmsInfo'>
           {{reqDate(date)}}
         </option>
      </select>
      <select @change='dateSort(sortType)' v-model='sortType'>
         <option value='all'>Дата</option>
         <option v-for='date in tempArray2' :value='date'>
           {{date}}
         </option>
      </select>
      <select @change='nameSort(sortType)' v-model='sortType'>
         <option value='all'>Фильм: все</option>
         <option v-for='date in filmsInfo' :value='date.film_name'>
           {{date.film_name}}
         </option>
      </select>
      <select style='display: none;'>
         <option>Зал: все</option>
         <option v-for='date in filmsInfo'>
           {{
              reqHall(date)
           }}
      ...
      <select @change='hallSort(sortType)' v-model='sortType'>
         <option value='all'>Зал: все</option>
         <option v-for='date in tempArray' :value='date'>
           {{
              date
           }}
         </option>
      ...
      <table v-for='schedul in schedule'>
      {{
          schedul[Object.keys(schedul)[Object.keys(schedul).length - 1]][0].filmTime.substr(0, 10)
      }}
      <tr>
        <td>ВРЕМЯ</td>
        <td>ФИЛЬМ</td>
      ...
      <tr v-for='schedu in schedul' :key='schedu.id'>
        <td>{{schedu[0].filmTime.slice(-5)}}</td>
        <td @click='getData(schedu)' style='cursor: pointer; text-decoration: underline;'>{{schedu[1].film_name}}</td>
        <td>{{schedu[0].filmHall}}</td>
        <td>{{schedu[0].filmPrice}}</td>
        <td>{{schedu[1].ticketBook}}</td>
      </tr>
      </table>
   </div>
    
    <div class='content__banner'>
       <img src='assets/dist/img/banner_afish.jpg'>
    </div>
...
loginMeth(){
      ...
      hallSort(sortType){
         ...
         if(sortType === 'all'){
            ...
            for(let i = 0; i < this.schedule.length; i++){
               for(let element in this.schedule[i]){
                  if(sortType !== this.schedule[i][element][0].filmHall){
                     delete this.schedule[i][element];
                     
                     if(isEmpty(this.schedule[i])){
      ...
      dateSort(sortType){
         ...
         if(sortType === 'all'){
            ...
            for(let i = 0; i < this.schedule.length; i++){
               for(let element in this.schedule[i]){
                  if(sortType !== this.schedule[i][element][0].filmTime.substr(0, 10)){
                     this.schedule.splice(i, 1);
                     i -= 1;
                     break;
                  }
      ...
      nameSort(sortType){
         ...
         if(sortType === 'all'){
            ...
            for(let i = 0; i < this.schedule.length; i++){
               for(let element in this.schedule[i]){
                  if(sortType !== this.schedule[i][element][1].film_name){
                     delete this.schedule[i][element];
                     if(isEmpty(this.schedule[i])){
                        this.schedule.splice(i, 1);
                        i -= 1;
      ...
      reqDate(date){
         for(let i = 0; i < date.filmInfo.length; i++){
            for(let e = 0; e < this.tempArray2.length; e++){
               if(date.filmInfo[i].filmTime.substr(0, 10) === this.tempArray2[e]){
                  break;
               } else if(e === this.tempArray2.length - 1){
                  this.tempArray2.push(date.filmInfo[i].filmTime.substr(0, 10));
                  if(this.tempArray2[0] === 0){
                  this.tempArray2.splice(0,1);
                  }
               }
            }
         }
      ...
      reqHall(date){
         for(let element in date.filmInfo){
            for(let i = 0; i < this.tempArray.length; i++){
               if(date.filmInfo[element].filmHall === this.tempArray[i]){
                 break;
               } else if(i === this.tempArray.length - 1){
                 this.tempArray.push(date.filmInfo[element].filmHall);
                 if(this.tempArray[0] === 0){
       ...
       getData(schedu){
         this.$router.push({name: 'Booking-a-ticket', params:{data: JSON.stringify(schedu)} });
       ...
       getUserFirstFunc(){
          ...
          get(child(dbRef, `users/`)).then((snapshot) => {
           ...
           get(child(dbRef, `banner_films/films_posters/`)).then((snapshot) => {
              let tempArray = [];
              snapshot.forEach(element => {
              tempArray.push(element.val());
              });
...
table>tr{
    ...
    border-right: 3px solid #FFD432;
    border-bottom: 0.5px solid #E8E8E8;
}

table>tr:first-child{
    border-top: 3px solid #FFD432;
}
