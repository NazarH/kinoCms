...
<div class='wrapper' id='wrapp'>
   ...
   <div class='content'>
       <div class='content-img'>
          <img :src='cinemaData.topBackPhot'>
       </div>
       <div class='content-blocks'>
          <div class='content-blocks-left'>
             <div class='banner-img'>
                <img src='assets/dist/img/book_banner.png'>
             </div>
             <h5>
                Количество залов: {{cinemaData.halls.length}}
             </h5>

             <table>
             <td>
             <tr v-for='hall in cinemaData.halls' :key='hall.id' class='hall-list' @click='getData(hall)'
             style='cursor: pointer;'>{{hall.hallNumb}}</tr>
             </td>
             </table>

             <div class='content-button-1'>
                <button><router-link to='/schedule'>Расписание всех сеансов</router-link></button>
             </div>
          </div>
          <div class='content-blocks-right'>
             <div class='content-top'>
                <h2>Оскар</h2>
                <img :src='cinemaData.logo'>
                <div class='content-button-2'>
                <button><router-link to='/schedule'>Расписание сеансов</router-link></button>
                <img src='assets/dist/img/dolby.png'>
                </div>
             </div>
             <div class='descr'>
                {{cinemaData.descr}}
             ...
             <h2>Условия</h2>
             <div class='cond'>
                {{cinemaData.conditions.substr(0, 2500)}}
             </div>
...
export default {
   ...
   created(){
       if(this.$route.params.data !== undefined){
           this.cinemaData = JSON.parse(this.$route.params.data);

           const db = getDatabase();
           set(fireRef(db, `cinemas-temp/0`), this.cinemaData);

           for(let element in this.cinemaData.galary){
               this.cinemaImages.push(this.cinemaData.galary[element]);
           }
           console.log(this.cinemaImages);
       } else {
           const dbRef = fireRef(getDatabase());

            get(child(dbRef, `cinemas-temp/`)).then((snapshot) => {
            var banners = [];
            snapshot.forEach(element => {
              banners.push(element.val());
            });
            this.cinemaData = banners[0];
   ...
   methods:{
      ...
      searchFilm(){
       ...
       getData(hall){
         this.$router.push({name: 'Hall', params:{data: JSON.stringify(hall)} });
         console.log(this.$router);
       },
       swipeLeft(){
           this.counter3--;
           if(this.counter3 < 0){
               this.counter3 = this.cinemaImages.length - 1;
           }
           document.getElementById('filmSwiper').src=`${this.cinemaImages[this.counter3].img}`;
       },
       swipeRight(){
           this.counter3++;
           if(this.counter3 > this.cinemaImages.length - 1){
               this.counter3 = 0;
           }
           document.getElementById('filmSwiper').src=`${this.cinemaImages[this.counter3].img}`;
       },
...
/* ---------------- 0 */
  ...
  .content-button-1>button{
      ...
      padding-top: 8px;
      padding-bottom: 8px;
      border: 3px solid #39522B;
      border-radius: 4px;
      margin-top: 30px;
  }

  .content-button-1>button>a{
      color: black;
  }

  .content-button-2>button>a{
      color: black;
  }
