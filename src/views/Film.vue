...
<div class='wrapper' id='wrapp'>
   ...
   <div class='content'>
      <div class='ytVideo'>
        <iframe width="1687" height="610" :src="filmData.scr_yt" 
...
export default {
   ...
   created(){
       if(this.$route.params.data !== undefined){
           ...
           set(fireRef(db, `banner_films/temp_film/0`), this.filmData);

           for(let element in this.filmData.img_list){
               this.filmDataImgs.push(this.filmData.img_list[element]);
            }
       } else {
           const dbRef = fireRef(getDatabase());

            get(child(dbRef, `banner_films/temp_film/`)).then((snapshot) => {
            var banners = [];
            snapshot.forEach(element => {
              banners.push(element.val());
            });
            this.filmData = banners[0];
   ...
   methods:{
      ...
      searchFilm(){
       ...
       swipeLeft(){
           this.counter3--;
           if(this.counter3 < 0){
               this.counter3 = this.filmDataImgs.length - 1;
           }
           document.getElementById('filmSwiper').src=`${this.filmDataImgs[this.counter3].img}`;
       },
       swipeRight(){
           this.counter3++;
           if(this.counter3 > this.filmDataImgs.length - 1){
               this.counter3 = 0;
           }
           document.getElementById('filmSwiper').src=`${this.filmDataImgs[this.counter3].img}`;
       },
