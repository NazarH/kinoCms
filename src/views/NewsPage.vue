      ...
      <div>
         <div class='content__left'>
            ...
            <div class='some-blocks'>
              <div class='stockInfo'>
               <img :src='newsData.newsMainImg'>
              </div>
              <div class='newsDescr'>
              {{newsData.newsDescr.substr(0, 1679)}}
               </div>

               
            </div>
            <div class='ytVideo'>
                 <iframe width="1000" height="515" :src="newsData.newsLink" 
   ...
   components:{
    ...
    created(){
       if(this.$route.params.data !== undefined){
           this.newsData = JSON.parse(this.$route.params.data);

           const db = getDatabase();
           set(fireRef(db, `news-temp/0`), this.newsData);
   ...
   methods:{
      ...
      searchFilm(){
       ...
       getData2(film){
        ...
        imgSwipe(){
         setTimeout(() => {
            document.getElementById('imgSwipe').src=`${this.newsData.newsGalary[this.counter].img}`;
            document.getElementById('btnSwipe').click();
            setTimeout(() => {
               this.counter = this.counter + 1;
            }, 2000);
            setTimeout(() => {
               if(this.counter === this.newsData.newsGalary.length){
               this.counter = 0;
               }
            }, 2500);
         }, 3000);
       ...
       getUserFirstFunc(){
          ...
          get(child(dbRef, `users/`)).then((snapshot) => {
            ...
            get(child(dbRef, `news-temp/`)).then((snapshot) => {
            var banners = [];
            snapshot.forEach(element => {
              banners.push(element.val());
            });
            this.newsData = banners[0];
