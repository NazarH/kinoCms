...
<div class='wrapper' id='wrapp'>
   ...
   <div class='content'>
       <div class='content-img'>
          <img :src='hallData.hallTopBackPhot'>
          <div>
       ...
       <div class='content-blocks'>
          ...
          <div class='content-blocks-right'>
             <h2>{{hallData.hallNumb}}</h2>
             <div class='descr'>
                {{hallData.hallDescr}}
             </div>
             <h2>Карта зала</h2>
             <img :src='hallData.hallLayout'>
             <h2>ФОТОГАЛЕРЕЯ</h2>
...
export default {
   ...
   created(){
       if(this.$route.params.data !== undefined){
           this.hallData = JSON.parse(this.$route.params.data);

           for(let element in this.hallData.hallGalary){
               this.hallImages.push(this.hallData.hallGalary[element]);
           }

           const db = getDatabase();
           set(fireRef(db, `cinemas-temp-hall/0`), this.hallData);
       } else {
   ...
   methods:{
      ...
      searchFilm(){
       ...
       swipeLeft(){
           this.counter3--;
           if(this.counter3 < 0){
               this.counter3 = this.hallImages.length - 1;
           }
           document.getElementById('filmSwiper').src=`${this.hallImages[this.counter3].img}`;
       },
       swipeRight(){
           this.counter3++;
           if(this.counter3 > this.hallImages.length - 1){
               this.counter3 = 0;
           }
           document.getElementById('filmSwiper').src=`${this.hallImages[this.counter3].img}`;
       },
       getUserFirstFunc(){
          ...
          get(child(dbRef, `users/`)).then((snapshot) => {
            ...
            get(child(dbRef, `cinemas-temp-hall/`)).then((snapshot) => {
            var banners = [];
            snapshot.forEach(element => {
              banners.push(element.val());
            });
            this.hallData = banners[0];
            console.log(this.hallData);

            for(let element in this.hallData.hallGallary){
               this.hallImages.push(this.hallData.hallGallary[element]);
            }
            document.getElementById('filmSwiper').src=`${this.hallImages[this.counter3].img}`;
            }); 
