<template>
    ...
    <div v-for='banner in obj_banners' :key='banner.id' class='banner'>
        <div class="square2">
        ...
        <div>URL:  <input v-bind:value='banner.img' readonly> </div> 
...
-->
    ...
    <form @submit.prevent name="myform" v-for='block in blocks' :key='block.id'>
        ...
        <div style='display: none;' id='inpt1'>URL: <input type='text' placeholder='Сохраните пост' id='sm_url' name='URL' readonly></div>
...
export default {
    ...
    methods:{
         getData() {
            ...
            set(fireRef(db, `banners/block/`), this.obj_banners);
             });
             } else {
               const db = getDatabase();
               set(fireRef(db, `banners/block/`), this.obj_banners);
        ...
        bannerDelete(banner){
          delete this.obj_banners[banner.id];
          console.log(this.obj_banners[banner.id])
        },
        ...
        onFileSelected(event){
          ...
          reader.onload = function(event){
            ...
            document.getElementById("sm_url").value = this.temp_src;
          }
        ...
        first_func(){
          ...
          get(child(dbRef, `banners/block/`)).then((snapshot) => {
            var banners = [];
            snapshot.forEach(element => {
              banners.push(element.val());
            });
            for(let i = 0; i < banners.length; i++){
              this.obj_banners[banners[i].id] = banners[i]; 
            }
          });
        }
    },
